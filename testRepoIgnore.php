...
function loadSummerNoteJS($args) {
    ...
    if ($Wcms->loggedIn) {
        ...
        <script src="{$Wcms->url('plugins/testRepoIgnore/js/admin.js')}" type="text/javascript"></script>
        <script src="{$Wcms->url('plugins/testRepoIgnore/js/files.js')}" type="text/javascript"></script>
EOT;
        $args[0] .= $script;
    }
    return $args;
...
function loadSummerNoteCSS($args) {
    ...
    if ($Wcms->loggedIn) {
        ...
        <link rel="stylesheet" href="{$Wcms->url('plugins/testRepoIgnore/css/admin.css')}" type="text/css" media="screen">
EOT;
        $args[0] .= $script;
    }
    return $args;
